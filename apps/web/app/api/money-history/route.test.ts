...
describe("GET /api/money-history (Story 5.1, FR31)", () => {
  ...
  // route -- this story is what first makes `/money-history` reachable via
  // the UI for that role. Mirrors the Partner case immediately above, one
  // role over: own Sub-partner Share's entries only, a sibling
  // Sub-partner's entries on the same Project excluded.
  it("a Sub-partner sees only entries matching their own current Sub-partner Share -- excluding a sibling Sub-partner's entries", async () => {
    sessionFor(SUB_PARTNER_USER);
    subPartnerSharesListAll.mockResolvedValue([SUB_PARTNER_SHARE_PROJECT_A, OTHER_SUB_PARTNER_SHARE]);
    investmentTransactionsListAll.mockResolvedValue([
      makeInvestmentTransaction({ id: "own", partyType: "sub_partner", shareId: "sub-partner-1" }),
      makeInvestmentTransaction({ id: "not-mine", partyType: "sub_partner", shareId: "sub-partner-2" }),
    ]);
    ...
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.entries.map((e: { id: string }) => e.id)).toEqual(["own"]);
  });
