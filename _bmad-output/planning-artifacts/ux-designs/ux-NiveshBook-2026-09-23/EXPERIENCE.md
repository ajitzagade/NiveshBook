...
> Draft, reconciled against a founder-provided screen-mockup artifact covering 11 screens (`imports/founder-mockup.html`) plus PRD §3.3/§6, NFR1-19, and the architecture spine. Expanded to all 14 IA surfaces in `mockups/all-screens-demo.html` (adds Users, Permissions, Audit History, plus a Trail Quick View) for stakeholder demo purposes. Component behavior below is grounded in that mockup where it shows one; anything the static mockup can't demonstrate (real interactions, empty/error states, the exact mobile collapse) stays `[ASSUMPTION]` pending a founder look. Paired with `DESIGN.md` (visual identity). Spines win on conflict with any mock or import.
...
| Partner Shares | Everyone (scoped) | View own share; Owner/Admin edits and validates against 100%. A `PARTNER_SHARE` record is keyed by `(shareId, projectId)` (architecture ERD) — shares are per-Project, not global, so two Projects can have fully different or only partly overlapping Partner rosters. Switching the current Project (via project search) re-scopes this screen, Add Money, and Withdraw Money to that Project's own Partners. |
...
| Project search | Sidebar, above the role switcher | Type-ahead against Project names; selecting one re-scopes Partner Shares, Add Money, and Withdraw Money to that Project's own Partner roster (no partial/stale data left over from the previous Project). `[ASSUMPTION]` demo uses a free-text field matching by exact name; the real build likely wants the Combobox pattern (Component Patterns → Searchable combobox) for consistency. |
| Partner detail | Reached by clicking a Partner's name anywhere it appears (Partner Shares, Add Money, Withdraw Money) | Scoped to exactly one Partner + their own Sub-partners — their share, their Add Money history, their Withdraw Money history. No other Partner's row ever renders on this surface, even for Owner/Admin (this is a focus tool, not a different permission level). A Partner/Sub-partner role viewing Partner Shares already sees only their own row+subs by default (FR9's privacy silo) — this screen is that same scoped view made explicit and reachable on demand, including for Owner/Admin who otherwise sees everyone. |
...
| Trail (vertical) + trace banner | Money History detail, linked-transaction navigation (FR-32) | Vertical timeline, one colored dot per node (color = that node's transaction type). A trace banner ("Trace ID: FLOW-A-1024... all child transactions retain their original withdrawal relationship") sits above the trail when tracing a specific chain (FR-28, FR-30). Default/primary trail pattern. `[ASSUMPTION]` exact trigger to enter trace mode from a list row is undecided — mockup shows it as a static example, not an interaction. |
| Trail Quick View (horizontal branching) | Money History detail — alternate view of the same trail, toggled via a "Trail Quick View" control | Same trail data as the vertical timeline, laid out left-to-right in generations so branching (one withdrawal splitting into several destinations, one of those splitting further) reads at a glance. Replaces the vertical trail in place when toggled on, per screen (`DESIGN.md.Components`) — never both at once. |
| Report tile | Reports | Icon badge + name + one-line description, in a 3-up grid — one tile per report type (FR-38). |
| Empty state | Any list/dashboard with no data yet | `packages/ui`'s `EmptyState` (added 2026-09-24): icon + one NiveshBook-specific heading and sentence in Glossary vocabulary + single primary action where one exists (e.g. "Create your first Project"). Not shown in the mockup (all screens are pre-populated) — copy is per-surface, written to match that screen's own voice, but the component shape itself is now standard, not a per-page improvisation. |
| Page header | Every screen | `packages/ui`'s `PageHeader` (added 2026-09-24): optional back-link, title, optional description, right-aligned primary action. Extracted after a code-level UI audit found four screens had already converged on this exact shape independently — centralizing it is what keeps a fifth screen from drifting (inconsistent margins, a differently-sized title, a misplaced action button). |
